...
import { Package2, Lightbulb, CheckCircle2, AlertTriangle, XCircle, ExternalLink, Bell, BellOff, Loader2, CalendarDays } from "lucide-react";
...
type TypeFilter = "All" | "ETB" | "Booster Box" | "Booster Bundle" | "Upcoming";
...
export default function Packs() {
  ...
  const [releases, setReleases] = useState<ReleasesState>({ status: "idle" });
  const [alerts, setAlerts] = useState<Record<string, boolean>>({});

  // Market prices + release alert toggles persist in localStorage
  useEffect(() => {
  ...
  // Fetch releases the first time the Upcoming tab is opened
  ...
  useEffect(() => {
    if (filter === "Upcoming" && releases.status === "idle") loadReleases();
  }, [filter, releases.status, loadReleases]);
  ...
  const visible = filter === "All" || filter === "Upcoming" ? PRODUCTS : PRODUCTS.filter(p => p.type === filter);
